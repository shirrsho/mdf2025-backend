...
export class TimeslotCrudService extends BaseService<TimeslotCrudService> {
  ...
  /**
   * Get available timeslots (not occupied by webinars)
   */
  async getAvailableTimeslots(): Promise<TimeslotDocument[]> {
   ...
   */
  async isTimeslotAvailable(timeslotId: string): Promise<boolean> {
   ...
   */
  async markTimeslotUnavailable(timeslotId: string): Promise<TimeslotDocument> {
    const updatedTimeslot = await this.timeslotModel
      .findByIdAndUpdate(
        timeslotId, 
        { isAvailable: false }, 
   ...
   */
  async markTimeslotAvailable(timeslotId: string): Promise<TimeslotDocument> {
    const updatedTimeslot = await this.timeslotModel
      .findByIdAndUpdate(
        timeslotId, 
        { isAvailable: true }, 
